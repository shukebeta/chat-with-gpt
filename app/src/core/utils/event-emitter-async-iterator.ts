...
export class EventEmitterAsyncIterator<T> implements AsyncIterableIterator<EventEmitterAsyncIteratorOutput<T>> {
  private readonly eventQueue: Array<EventEmitterAsyncIteratorOutput<T>> = []
  private readonly resolveQueue: Array<(value: IteratorResult<EventEmitterAsyncIteratorOutput<T>>) => void> = []
  ...
  /**
     ...
     */
  constructor (private readonly eventEmitter: EventEmitter, eventNames: string[]) {
  ...
  async next (): Promise<IteratorResult<EventEmitterAsyncIteratorOutput<T>>> {
    if (this.eventQueue.length > 0) {
      ...
      return await new Promise<IteratorResult<EventEmitterAsyncIteratorOutput<T>>>(resolve => {
