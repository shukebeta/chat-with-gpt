...
import { type Chat, type Message, type Parameters, type UserSubmittedMessage } from './chat/types'
...
export class ChatManager extends EventEmitter {
  ...
  private readonly activeReplies = new Map<string, ReplyRequest>()
  ...
  private attachYDoc (username: string) {
    ...
    pluginRunner(
      ...
      async plugin => { await plugin.initialize() }
    )
    ...
    if (username !== 'anonymous') {
      ...
      provider?.whenSynced.then(() => {
        if (doc) {
          ...
          setTimeout(async () => { await provider.clearData() }, 10 * 1000)
        }
      })
    }
