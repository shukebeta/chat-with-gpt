...
import type WebSpeechPlugin from '../../tts-plugins/web-speech'

export default class DirectTTSPlayer extends AbstractTTSPlayer {
  playing = false
  ended = false

  private readonly loop: AsyncLoop
  private currentIndex = 0
  private currentPlaybackIndex = 0

  private promises: any[] = []

  constructor (private readonly plugin: WebSpeechPlugin) {
    super()
    console.log('tts init, directttsplayer')

    this.emit('state', this.getState())

    this.loop = new AsyncLoop(async () => { await this.tick() }, 100)
    this.loop.start()
  }
