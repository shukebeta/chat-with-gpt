import { AbstractTTSPlayer, type TTSPlayerState } from './types'
import { cloneArrayBuffer, md5, sleep } from '../utils'
import { AsyncLoop } from '../utils/async-loop'
import * as idb from '../utils/idb'
import type TTSPlugin from './tts-plugin'
...
export function resetAudioContext () {
  if (audioContextInUse) {
    ...
    setTimeout(async () => { await previousAudioContext.close() }, 0)
    setTimeout(async () => { await audioContext.suspend() }, 100)
  }
  audioContext.resume()
}
...
export default class ExternalTTSAudioFilePlayer extends AbstractTTSPlayer {
  ...
  private readonly downloadLoop: AsyncLoop
  private readonly schedulerLoop: AsyncLoop
  ...
  constructor (private readonly plugin: TTSPlugin) {
    super()
  ...
  private readonly schedule = async () => {
