...
interface MergeRange { start: number, end: number }
...
function bytePairMerge (piece: Uint8Array, ranks: RankMap): MergeRange[] {
  const parts: MergeRange[] = Array.from({ length: piece.length }, (_, i) => ({ start: i, end: i + 1 }))
  while (true) {
    if (parts.length === 1) {
      break
    }
...
export class CoreBPE {
  ...
  constructor (
    ...
    const specialTokensDecoder = new Map<number, Uint8Array>(
