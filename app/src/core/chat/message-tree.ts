...
export class MessageTree {
  public messageNodes = new Map<string, MessageNode>() // TODO make private

  constructor (messages: Array<Message | MessageNode> = []) {
  ...
  /**
     ...
     */
  public serialize (): Array<Omit<MessageNode, 'parent' | 'replies'>> {
    return Array.from(this.messageNodes.values())
      .map((messageNode) => {
